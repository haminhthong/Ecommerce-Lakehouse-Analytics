...
CERTIFIED_GOLD_COLUMNS: set[str] = {
    "Order_ID",
    "Order_Date",
    "Customer_ID",
    "Product_Name",
...
def validate_business_values(df: pd.DataFrame) -> QualityReportSummary:
    ...
    if {"Net_Line_Amount", "Gross_Profit"}.issubset(df.columns):
        return validate_certified_gold_values(df)
...
def validate_certified_gold_values(df: pd.DataFrame) -> QualityReportSummary:
    """Kiểm tra dataset Gold trước khi sinh báo cáo BI."""
    required_columns_ok = CERTIFIED_GOLD_COLUMNS.issubset(df.columns)
    column_result = QualityResult(
        rule="certified_gold_columns",
        passed=required_columns_ok,
        detail=(
            "Đủ các cột Gold bắt buộc"
            if required_columns_ok
            else "Thiếu các cột Gold: "
            + ", ".join(sorted(CERTIFIED_GOLD_COLUMNS.difference(df.columns)))
        ),
        severity="ERROR",
    ...
    required_nulls = df[list(CERTIFIED_GOLD_COLUMNS)].isna().any(axis=1)
    invalid_amounts = (
        df["Quantity"].le(0)
